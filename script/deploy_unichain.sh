...
AUTOMATION_OWNER="${AUTOMATION_OWNER:-$DEPLOYER_ADDR}"
TIMELOCK_ADMIN="${TIMELOCK_ADMIN:-$DEPLOYER_ADDR}"
TIMELOCK_PROPOSER="${TIMELOCK_PROPOSER:-$DEPLOYER_ADDR}"
TIMELOCK_EXECUTOR="${TIMELOCK_EXECUTOR:-$DEPLOYER_ADDR}"
ROLE_ADMIN="${ROLE_ADMIN:-}"
REACTIVE_EXECUTOR="${REACTIVE_EXECUTOR:-$DEPLOYER_ADDR}"
EMERGENCY_ACTOR="${EMERGENCY_ACTOR:-$DEPLOYER_ADDR}"
...
SELF_CONDITION=$(cast abi-encode "f((uint8,bool,bool,uint64))" "($POLICY_MIN_AGE,$POLICY_REQUIRE_CONTRIBUTOR,$POLICY_REQUIRE_DAO_MEMBER,$POLICY_MAX_ATTESTATION_AGE)")
