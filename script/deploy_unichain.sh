...
AGENT_ADAPTER_ID="${AGENT_ADAPTER_ID:-$(cast keccak "AGENT_LINK")}" 
...
AGENT_POLICY_CONTEXT="${AGENT_POLICY_CONTEXT:-$(cast keccak "AGENT_POLICY_CONTEXT_V1")}"
AGENT_MAX_PROOF_AGE="${AGENT_MAX_PROOF_AGE:-300}"
AGENT_REQUIRED_SOURCE_BRIDGE_ID="${AGENT_REQUIRED_SOURCE_BRIDGE_ID:-$(cast keccak "SATISFY_AGENTKIT_WORLD_V1")}"
...
AGENT_TRUSTED_SIGNER="${AGENT_TRUSTED_SIGNER:-$DEPLOYER_ADDR}"

if [[ "$DEPLOY_V2_AGENTKIT" == "true" && -z "$ARTIFACT_SUFFIX" ]]; then
  ARTIFACT_SUFFIX="-v2-agentkit"
fi
...
AGENT_ADAPTER_ADDR=""
if [[ "$DEPLOY_V2_AGENTKIT" == "true" ]]; then
  log "Deploying AgentLinkAdapter (V2)"
  deploy_contract "src/adapters/AgentLinkAdapter.sol:AgentLinkAdapter" "$DEPLOYER_ADDR" "$AGENT_TRUSTED_SIGNER"
  AGENT_ADAPTER_ADDR="$LAST_DEPLOYED_ADDR"
fi
...
if [[ "$DEPLOY_V2_AGENTKIT" == "true" ]]; then
  send_tx "$ENGINE_ADDR" "registerAdapter(bytes32,address)" "$AGENT_ADAPTER_ID" "$AGENT_ADAPTER_ADDR"
fi
...
if [[ "$DEPLOY_V2_AGENTKIT" == "true" ]]; then
  AGENT_CONDITION=$(cast abi-encode "f((bool,bytes32,uint64,bytes32))" "(true,$AGENT_POLICY_CONTEXT,$AGENT_MAX_PROOF_AGE,$AGENT_REQUIRED_SOURCE_BRIDGE_ID)")
  GROUP_WORLD_SELF="[(${WORLD_ADAPTER_ID},${WORLD_CONDITION}),(${SELF_ADAPTER_ID},${SELF_CONDITION})]"
  GROUP_AGENT="[(${AGENT_ADAPTER_ID},${AGENT_CONDITION})]"
  POLICY_GROUPS="[${GROUP_WORLD_SELF},${GROUP_AGENT}]"

  log "Creating V2 DNF policy: (World+Self) OR AgentLink"
  send_tx "$ENGINE_ADDR" "createPolicyV2(((bytes32,bytes)[])[],uint64,uint64,bool)" "$POLICY_GROUPS" "$POLICY_START_TIME" "$POLICY_END_TIME" "$POLICY_ACTIVE"
else
...
if [[ "$DEPLOY_V2_AGENTKIT" == "true" ]]; then
  send_tx "$AGENT_ADAPTER_ADDR" "transferOwnership(address)" "$AUTOMATION_MODULE_ADDR"
fi
...
if [[ "$DEPLOY_V2_AGENTKIT" == "true" ]]; then
  assert_owner "$AGENT_ADAPTER_ADDR" "$AUTOMATION_MODULE_ADDR" "AgentLinkAdapter"
fi
...
if [[ "$DEPLOY_V2_AGENTKIT" == "true" ]]; then
  AGENT_ADAPTER_JSON_VALUE="\"$AGENT_ADAPTER_ADDR\""
  AGENT_ADAPTER_ID_JSON_VALUE="\"$AGENT_ADAPTER_ID\""
  AGENT_ADAPTER_ID_OUTPUT="$AGENT_ADAPTER_ID"
else
  AGENT_ADAPTER_JSON_VALUE="null"
  AGENT_ADAPTER_ID_JSON_VALUE="null"
  AGENT_ADAPTER_ID_OUTPUT=""
fi

mkdir -p "$ROOT_DIR/deployments"
DEPLOYMENT_FILE="$ROOT_DIR/deployments/unichain-${UNICHAIN_NETWORK}${ARTIFACT_SUFFIX}.json"
...
cat > "$DEPLOYMENT_FILE" <<JSON
{
  ...
  "agentAdapter": $AGENT_ADAPTER_JSON_VALUE,
  ...
  "agentAdapterId": $AGENT_ADAPTER_ID_JSON_VALUE,
  "poolId": "$POOL_ID",
  "policyId": "$POLICY_ID",
  "epoch": "$CURRENT_EPOCH",
  "v2AgentKitEnabled": "$DEPLOY_V2_AGENTKIT",
  "governance": {
  ...
  "verifierConfig": {
    ...
    "agentPolicyContext": "$AGENT_POLICY_CONTEXT",
    "agentMaxProofAge": "$AGENT_MAX_PROOF_AGE",
    "agentRequiredSourceBridgeId": "$AGENT_REQUIRED_SOURCE_BRIDGE_ID",
    "agentTrustedSigner": "$AGENT_TRUSTED_SIGNER",
...
if [[ "$DEPLOY_V2_AGENTKIT" == "true" ]]; then
  log "AgentAdapter:      $AGENT_ADAPTER_ADDR"
fi
...
VITE_UNICHAIN_SEPOLIA_AGENT_ADAPTER_ID=$AGENT_ADAPTER_ID_OUTPUT
VITE_AGENT_ADAPTER_ID=$AGENT_ADAPTER_ID_OUTPUT
...
VITE_UNICHAIN_MAINNET_AGENT_ADAPTER_ID=$AGENT_ADAPTER_ID_OUTPUT
VITE_AGENT_ADAPTER_ID=$AGENT_ADAPTER_ID_OUTPUT
